...
import { ScrollView, TextInput, ActivityIndicator, Text, ImageBackground, StyleSheet, View } from 'react-native';
  ...
  const registerUser = async () => {
    ...
    firebase().createUserWithEmailAndPassword(email, password)
      .then(() => {
        ...
        setIndicatiorStatus(false);
        setEmailErr(error.userInfo['message']);
      });
  };
  ...
  return (
    <ScrollView>
      <ImageBackground source={image} style={style.imageBackground}>
        {
          indicatiorStatus ?
            <View style={style.activityViewContainer}>
...
const style = StyleSheet.create({
  ...
  activityViewContainer: {
    flex: 1, justifyContent: 'center', alignItems: 'center', marginTop: hp('30%')
  }
});
